...
import { TerminalBugExecutor, type TerminalBugCommand } from './terminal-bug-executor.js';
...
export class BridgeDaemon {
  ...
  private terminalBugExecutor?: TerminalBugExecutor;
  ...
  async start(): Promise<void> {
    ...
    if (this.workspaces.length > 0) {
      ...
      // Create TerminalBugExecutor (new architecture: Bridge as infrastructure, Claude as actor)
      this.terminalBugExecutor = new TerminalBugExecutor(bugExecutorClient, this.config);
      this.log('[Daemon] TerminalBugExecutor created (Claude-as-actor mode)');
    } else {
  ...
  private async heartbeat(): Promise<void> {
    // Enhanced heartbeat with connection health logging (AutonomousBugFixer v1.0)
    const connectedChannels = this.channels.filter(ch => ch.state === 'joined').length;
    const totalChannels = this.channels.length;

    // Log connection health every heartbeat
    if (connectedChannels < totalChannels) {
      this.log(`[Health] Connection degraded: ${connectedChannels}/${totalChannels} channels connected`);
    }
    ...
    // If heartbeat fails, log but don't crash - VPS connectivity resilience
    if (error) {
      this.log(`[Health] Heartbeat update failed: ${error.message}`);
    }
  }
  ...
  private async reconnect(): Promise<void> {
    ...
    for (const channel of this.channels) {
      try {
        ...
        this.log(`[Reconnect] Failed to remove channel: ${e}`);
      }
    }
    this.channels = [];
    this.channel = null;
    ...
    // If too many reconnect attempts, exponential backoff with longer delay
    if (this.reconnectAttempts > this.maxReconnectAttempts) {
      const backoffMs = Math.min(60000 * this.reconnectAttempts, 300000); // Max 5 min
      this.log(`[Reconnect] Too many attempts. Backing off for ${backoffMs/1000}s`);
      await new Promise(resolve => setTimeout(resolve, backoffMs));
      this.reconnectAttempts = 0; // Reset after successful backoff
    }

    // Resubscribe
    try {
      await this.subscribe();
      this.log('[Reconnect] Subscription restored successfully');
      this.reconnectAttempts = 0; // Reset on success
    } catch (e) {
      this.log(`[Reconnect] Subscription failed: ${e}`);
  ...
  private async handleCommand(command: Command): Promise<void> {
    ...
    if (claimed === 'already_mine') {
      // Already claimed by us but not in processingCommands - another instance is handling it
      this.log(`[handleCommand] Skipping ${command.id} - already being executed`);
      return;
    }
    ...
    try {
      ...
      if (command.command_type === 'bug_execution' && this.terminalBugExecutor) {
        // Delegate bug_execution to TerminalBugExecutor (Claude-as-actor pattern)
        // Bridge becomes infrastructure: writes bug-context.md, spawns Claude with mentu CLI
        // Claude becomes actor: has full autonomy to fix bug and close commitment
        this.log(`[handleCommand] Routing ${command.id} to TerminalBugExecutor (command_type: bug_execution)`);

        const terminalCommand: TerminalBugCommand = {
          id: command.id,
          workspace_id: command.workspace_id,
          working_directory: command.working_directory,  // Use directly, no resolution
          payload: command.payload as { memory_id: string; commitment_id?: string; timeout_seconds?: number },
          timeout_seconds: command.timeout_seconds,
        };

        const terminalResult = await this.terminalBugExecutor.execute(terminalCommand);

        // Adapt TerminalBugExecutor result to daemon's ExecutionResult format
        result = {
          status: terminalResult.success ? 'success' : 'failed',
          exit_code: terminalResult.exit_code,
          stdout: terminalResult.stdout,
          stderr: terminalResult.stderr,
          error_message: terminalResult.success ? undefined : terminalResult.stderr,
        };
        this.log(`[handleCommand] TerminalBugExecutor completed: ${terminalResult.success ? 'success' : 'failed'} (${terminalResult.duration_ms}ms)`);
      } else if (command.command_type === 'bug_execution' && this.bugExecutor) {
        // Fallback to legacy BugExecutor (Auditor+Executor pattern) if TerminalBugExecutor not available
        this.log(`[handleCommand] Routing ${command.id} to BugExecutor (legacy, command_type: bug_execution)`);
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.terminalBugExecutor) {
      await this.terminalBugExecutor.stop();
    }
